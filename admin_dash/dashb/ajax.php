...
if (isset($_POST['login'])) {
    ...
    if (!$email && !$password) {
        ...
        // $password = md5($password);  // create 16 or 32 character hex and binary format
...
///Coupon system discount for any% 
if (isset($_POST['room_price'])) {
    ...
    if ($result) {
        ...
        $discountedPrice = $row['price'] - $row['price'] * 20/100;    // 20% discount given on original room rate of a single room.
...
if (isset($_POST['createManager'])) {                    /// createManager here
    $name = $_POST['name'];
    ...
    $result = mysqli_query($connection, $query);
    if ($result) {
        header("Location:index.php?createmanager&success");
    } else {
        header("Location:index.php?createmanager&error");
    }
...
if (isset($_GET['delete_manager'])) {
    ...
    $sql = "DELETE FROM user WHERE id='$manager_id'";
    $result = mysqli_query($connection, $sql);
    if ($result) {
        header("Location:index.php?createmanager&deleteSuccess");
    } else {
        header("Location:index.php?createmanager&deleteError");
    }
}
